...
# Manual scenario:
# History defines the real trajectory used to obtain filtered state at t0.
# Rollout actions are then imagined from that filtered state.
...
# This gives filtered history final state around the classic difficult region.

ROLLOUT_ACTIONS = [
    0, 0, 2, 0, 0, 0, 0, 2, 0, 0, 0, 2, 0, 0, 3
]

OUTPUT_DIR = "./debug_v2_outputs"
...
def build_history_from_actions(
    ...
    """
    Generate real history by stepping the true environment.
    """
    ...
    return {
        "observations": np.array(observations, dtype=np.float32),  # [T, H, W]
        "rows": np.array(rows, dtype=np.int64),                    # [T]
        "cols": np.array(cols, dtype=np.int64),                    # [T]
        "headings": np.array(headings, dtype=np.int64),            # [T]
        "collisions": np.array(collisions, dtype=np.int64),        # [T-1]
    }
...
def rollout_ground_truth_from_env(
    ...
    """
    Roll forward the true env from its current state.
    """
    rows = []
    cols = []
    headings = []
...
def main() -> None:
    ...
    # After history, env is now at final true filtered state.
    ...
    # observations shape currently [1,T,H,W]? let's fix carefully:
    # history["observations"] is [T,H,W], so unsqueeze(0)->[1,T,H,W], unsqueeze(2)->[1,T,1,H,W]

    actions = torch.tensor(HISTORY_ACTIONS, dtype=torch.long, device=device).unsqueeze(0)  # [1,T-1]

    with torch.no_grad():
        filt = model.forward_filter(observations, actions)

        row_logits_seq = filt["row_logits_seq"][0]         # [T,R]
        col_logits_seq = filt["col_logits_seq"][0]         # [T,C]
        heading_logits_seq = filt["heading_logits_seq"][0] # [T,Hd]

        row_pred_seq = row_logits_seq.argmax(dim=-1).cpu().numpy()
        col_pred_seq = col_logits_seq.argmax(dim=-1).cpu().numpy()
        heading_pred_seq = heading_logits_seq.argmax(dim=-1).cpu().numpy()

    # --------------------------------------------------------
    # Print filtered-history debug
    # --------------------------------------------------------
    T = len(history["rows"])
    print("\n" + "=" * 130)
    print("FILTERED HISTORY DEBUG (V2)")
    print("=" * 130)
    ...
    for t in range(T):
        ...
        print(
            f"{t:2d} | "
            f"{pose_tuple_str(tr, tc, th):16s} | "
            f"{pose_tuple_str(pr, pc, ph):16s} | "
    ...
    # --------------------------------------------------------
    # V2 rollout from filtered state
    # --------------------------------------------------------
    z_start = filt["z_seq"][:, t0, :]    # [1,Z]
    h_start = filt["h_seq"][:, t0, :]    # [1,H]
    action_roll = torch.tensor(ROLLOUT_ACTIONS, dtype=torch.long, device=device).unsqueeze(0)  # [1,K]
    ...
    print("\n" + "=" * 130)
    print("MANUAL TRANSITION DEBUG (V2)")
    print("=" * 130)
