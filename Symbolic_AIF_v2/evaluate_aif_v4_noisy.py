# evaluate_aif_v4_noisy.py
#
# Noise stress-test for WorldModelV4 + EFEPlannerV2.
# Runs the closed-loop agent across multiple noise conditions to measure how
# well V4's uncertainty representation degrades gracefully under perceptual noise.
#
...
#
# Key optimization: start/goal pairs and distance maps are pre-computed ONCE
# and reused across all noise conditions so results are directly comparable.
...
def apply_noise(
    ...
    """Apply noise in-place to a float32 [0,1] observation array."""
    out = obs.copy()
    if gauss_sigma > 0.0:
...
def build_model(cfg: SweepConfig, device: torch.device) -> WorldModelV4:
...
def obs_to_tensor(obs_history: List[np.ndarray], device: torch.device) -> torch.Tensor:
    arr = np.stack(obs_history, axis=0).astype(np.float32)
    return torch.from_numpy(arr).unsqueeze(0).unsqueeze(2).to(device)


def acts_to_tensor(action_history: List[int], device: torch.device) -> Optional[torch.Tensor]:
...
# ============================================================
# Single trial under a noise condition
# ============================================================
...
def run_trial(
    ...
    def corrupt(o: np.ndarray) -> np.ndarray:
        return apply_noise(o, noise.name, noise.gauss_sigma, noise.mask_fraction, noise_rng)

    obs_history:    List[np.ndarray]          = [corrupt(np.asarray(obs_raw, dtype=np.float32))]
    ...
    for a in cfg.warmup_actions:
        result = env.step(a)
        obs_history.append(corrupt(np.asarray(result.obs, dtype=np.float32)))
    ...
    for _ in range(cfg.max_steps):
        obs_t = obs_to_tensor(obs_history, device)
        act_t = acts_to_tensor(action_history, device)
...
def print_comparison_table(summaries: List[Dict[str, Any]]) -> None:
    ...
    print()
    print("=" * 95)
    print("NOISE STRESS-TEST — COMPARISON TABLE")
    print("=" * 95)
    hdr = (
    ...
    print("  Δ           : success rate change vs clean baseline (negative = degradation)")
    ...
    print("  CtxStd      : mean context VAE std per step (higher → model knows it's uncertain)")
    print("  PathEff     : steps_optimal / steps_taken for successful trials")
    print()
    print("What to look for:")
    print("  Good AIF:  Entropy and CtxStd both RISE under noise, success degrades gradually")
    print("  Bad AIF:   Model stays confident (low entropy) but belief errors spike → blind failure")
...
def main() -> None:
    ...
    print(f"History keep : {cfg.history_keep}")
    ...
    # Pre-compute pairs and distance maps ONCE (reused across all conditions)
    ...
    dist_maps: Dict[Tuple[int,int], torch.Tensor] = {}
    for i, (_, goal_pos) in enumerate(pairs):
    ...
    with open(summary_path, "w") as f:
        json.dump({"conditions": all_summaries, "total_wall_clock_seconds": round(total_elapsed, 1)}, f, indent=2)
