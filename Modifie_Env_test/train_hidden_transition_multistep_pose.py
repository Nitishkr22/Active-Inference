## train_hidden_transition_multistep_pose.py ##
...
class HiddenTransitionModelMultiStepPose(nn.Module):
    """
    Predict next GRU hidden state from current hidden state and action.
    """
...
def rollout_multistep_pose_loss(
    ...
    # Need h_seq[:, start_t + rollout_len, :]
    # so start_t + rollout_len <= T - 1
    ...
    for start_t in range(max_start + 1):
        # Starting hidden state is the true hidden state at time start_t
        h_pred = h_seq[:, start_t, :]   # [B, H]

        for k in range(rollout_len):
            # Action taken from time (start_t + k) to reach next state
...
def evaluate_pose_rollout_accuracy(
    ...
    """
    Compute rollout pose accuracy over all valid rollout steps.
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch >= 3 and val_loss < best_val_loss:
            ...
            torch.save(
                ...
                os.path.join(save_dir, "best_hidden_transition_multistep_pose.pt"),
            )

    print("Multistep pose-supervised transition training finished.")
    print("Best val loss:", best_val_loss)


if __name__ == "__main__":
    train_hidden_transition_multistep_pose()